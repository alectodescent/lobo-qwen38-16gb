...
static bool test_kvarn_partial_checkpoint_history(
    ...
    if (kvarn_frame == std::string::npos || version != 17) {
        LOG_ERR("%s: expected KVarN partial checkpoint format v17, found v%u\n", __func__, version);
        return false;
    }

    auto corrupt = checkpoint_b;
    const uint32_t unsupported_version = 18;
    ...
    std::unique_ptr<llama_state_seq_restore_plan, decltype(&llama_state_seq_restore_plan_free)> corrupt_plan(
            ...
            llama_state_seq_restore_plan_free);
    if (corrupt_plan) {
        LOG_ERR("%s: corrupt KVarN v17 frame produced a restore plan\n", __func__);
        return false;
    }
    if (llama_state_seq_set_data_ext(
            context.get(), corrupt.data(), corrupt.size(), 0, partial_flags) != 0) {
        LOG_ERR("%s: corrupt KVarN v17 frame was accepted\n", __func__);
        return false;
    }
    ...
    const int64_t live_pos_before_layout_reject =
            llama_memory_seq_pos_max(llama_get_memory(context.get()), 0);
    std::vector<uint8_t> live_state_before_layout_reject(
            llama_state_seq_get_size_ext(context.get(), 0, self_flags));
    if (live_state_before_layout_reject.empty() || llama_state_seq_get_data_ext(
                context.get(),
                live_state_before_layout_reject.data(),
                live_state_before_layout_reject.size(),
                0,
                self_flags) != live_state_before_layout_reject.size()) {
        LOG_ERR("%s: failed to snapshot live state before record-layout rejection\n", __func__);
        return false;
    }

    auto layout_mismatch = checkpoint_b;
    uint32_t saved_record_layout = 0;
    const size_t record_layout_offset = kvarn_frame + 4*sizeof(uint32_t);
    std::memcpy(&saved_record_layout, layout_mismatch.data() + record_layout_offset, sizeof(saved_record_layout));
    const uint32_t mismatched_record_layout = saved_record_layout == 0 ? 1 : 0;
    std::memcpy(
            layout_mismatch.data() + record_layout_offset,
            &mismatched_record_layout,
            sizeof(mismatched_record_layout));
    std::unique_ptr<llama_state_seq_restore_plan, decltype(&llama_state_seq_restore_plan_free)> layout_plan(
            llama_state_seq_prepare_data_ext(
                    context.get(), layout_mismatch.data(), layout_mismatch.size(), 0, partial_flags),
            llama_state_seq_restore_plan_free);
    if (layout_plan || llama_state_seq_set_data_ext(
            context.get(), layout_mismatch.data(), layout_mismatch.size(), 0, partial_flags) != 0) {
        LOG_ERR("%s: mismatched KVarN record layout was accepted\n", __func__);
        return false;
    }
    if (llama_memory_seq_pos_max(llama_get_memory(context.get()), 0) != live_pos_before_layout_reject) {
        LOG_ERR("%s: rejected KVarN record layout changed live metadata\n", __func__);
        return false;
    }
    std::vector<uint8_t> live_state_after_layout_reject(
            llama_state_seq_get_size_ext(context.get(), 0, self_flags));
    if (live_state_after_layout_reject.size() != live_state_before_layout_reject.size() ||
            llama_state_seq_get_data_ext(
                context.get(),
                live_state_after_layout_reject.data(),
                live_state_after_layout_reject.size(),
                0,
                self_flags) != live_state_after_layout_reject.size() ||
            live_state_after_layout_reject != live_state_before_layout_reject) {
        LOG_ERR("%s: rejected KVarN record layout changed live state bytes\n", __func__);
        return false;
    }
    ...
    LOG("\n");
    LOG("\nPASS: KVarN v17 host partial checkpoints are transactional and layout-tagged\n");
    return true;
}
