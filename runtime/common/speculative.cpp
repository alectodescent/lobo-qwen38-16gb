...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    std::vector<bool> recycle_started;
        ...
        , params(params.draft)
    {
        ...
        const char * skip_prefill_env = std::getenv("LLAMA_MTP_SKIP_PREFILL");
        skip_prefill = skip_prefill_env != nullptr && skip_prefill_env[0] == '1';
        if (skip_prefill) {
            SPC_INF("%s", "MTP prompt catch-up is bounded to the draft window; oversized prompts will seed from target hidden state\n");
        }
        ...
        recycle_started.assign(n_seq, false);
    }
    ...
    void begin(llama_seq_id seq_id, const llama_tokens & prompt) override {
        ...
        if (pos_max < N - 1 && !is_mem_shared && !skip_prefill) {
    ...
    bool recycle_draft_context(llama_seq_id seq_id, llama_pos n_past) {
        if (!skip_prefill || is_mem_shared) {
            return true;
        }
        ...
        const llama_pos n_reserve     = params.n_max + 1;
        const llama_pos n_ctx_storage = llama_n_ctx_seq(ctx_dft);
        if (n_ctx_storage <= n_reserve) {
            SPC_ERR("MTP context size %d is too small for %d reserved positions\n",
                    (int) n_ctx_storage, (int) n_reserve);
            return false;
        }

        const llama_pos p1 = n_past - (n_ctx_storage - n_reserve);
        if (p1 <= 0) {
            return true;
        }

        const llama_pos pos_min = llama_memory_seq_pos_min(memory, seq_id);
        if (pos_min < 0 || pos_min >= p1) {
            return true;
        }

        llama_pos planned_p0 = -1;
        llama_pos planned_p1 = -1;
        const bool can_remove_prefix = llama_memory_seq_rm_plan(
                memory, seq_id, 0, p1, &planned_p0, &planned_p1) &&
            planned_p0 == 0 && planned_p1 == p1;

        if (can_remove_prefix) {
            if (!llama_memory_seq_rm(memory, seq_id, planned_p0, planned_p1)) {
                SPC_ERR("failed to recycle MTP context for seq_id=%d before position %d\n",
                        (int) seq_id, (int) p1);
                return false;
            }
        } else {
            // KVarN stores immutable 128-token bodies and therefore cannot
            // evict an arbitrary prefix.  The MTP cache is only a proposal
            // accelerator, so restart that cache at the current absolute
            // position instead of weakening or materializing the target KV.
            // The target model still verifies every proposed token.
            if (!llama_memory_seq_rm(memory, seq_id, 0, -1)) {
                SPC_ERR("failed to reset bounded MTP context for seq_id=%d\n", (int) seq_id);
                return false;
            }
            SPC_INF("bounded MTP cache for seq_id=%d does not support prefix eviction; restarting draft history\n",
                    (int) seq_id);
        }

        if (!recycle_started[seq_id]) {
            SPC_INF("bounded MTP recycling is active for seq_id=%d (capacity=%d, reserve=%d, prefix_evict=%d)\n",
                    (int) seq_id, (int) n_ctx_storage, (int) n_reserve, (int) can_remove_prefix);
            recycle_started[seq_id] = true;
        }
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        // Keep prompt history when it fits in the bounded draft cache.  Large
        // prompts must not make the draft context scale with the target, but
        // throwing away every short prompt needlessly damages acceptance.
        const llama_pos n_ctx_storage = llama_n_ctx_seq(ctx_dft);
        const llama_pos n_reserve     = params.n_max + 1;
        bool prompt_exceeds_window = false;
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            if (i_batch_end[seq_id] >= 0 &&
                batch_in.pos[i_batch_end[seq_id]] >= n_ctx_storage - n_reserve) {
                prompt_exceeds_window = true;
                break;
            }
        }
        const bool skip_catchup = skip_prefill && prompt_exceeds_window && n_tokens > params.n_max + 1;
        if (skip_catchup && !is_mem_shared) {
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                if (i_batch_beg[seq_id] >= 0) {
        ...
        if (!is_mem_shared && !skip_catchup) {
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                if (i_batch_beg[seq_id] >= 0 &&
                    !recycle_draft_context(seq_id, batch_in.pos[i_batch_beg[seq_id]])) {
                    return false;
                }
            }
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            if (!recycle_draft_context(seq_id, dp.n_past)) {
                dp.drafting = false;
                continue;
            }
