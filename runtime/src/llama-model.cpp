...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // Legacy DFlash2 GGUFs may retain selector and convolution training
    // auxiliaries that are not part of the serving graph.  The historical
    // DFlash runtime ignored those tensors; keep strict complete loading for
    // every other architecture while accepting that established sidecar
    // format here.
    ml.done_getting_tensors(arch == LLM_ARCH_DFLASH);
