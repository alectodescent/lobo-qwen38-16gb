...
// Version 17 records the backend's physical code-payload layout. This prevents
// row-major state from being restored as the byte-neutral SM120 tile16 layout.
...
constexpr uint32_t KVAR_N_STATE_VERSION = 17;
...
uint32_t llama_kvarn_backend_record_layout(ggml_backend_dev_t dev) {
    if (dev == nullptr || ggml_backend_dev_type(dev) == GGML_BACKEND_DEVICE_TYPE_CPU) {
        return GGML_BACKEND_KVARN_RECORD_LAYOUT_ROW_MAJOR;
    }
    if (ggml_backend_dev_is_meta(dev)) {
        const size_t count = ggml_backend_meta_device_count(dev);
        uint32_t layout = GGML_BACKEND_KVARN_RECORD_LAYOUT_ROW_MAJOR;
        for (size_t i = 0; i < count; ++i) {
            const uint32_t candidate = llama_kvarn_backend_record_layout(
                    ggml_backend_meta_device_get(dev, i));
            if (i == 0) {
                layout = candidate;
            } else if (candidate != layout) {
                throw std::runtime_error("KVarN meta backend mixes incompatible record layouts");
            }
        }
        return layout;
    }
    if (auto * capabilities_fn = kvarn_capabilities_proc(dev)) {
        ggml_backend_kvarn_capabilities capabilities = {};
        if (query_kvarn_capabilities(dev, capabilities_fn, capabilities)) {
            return capabilities.record_layout;
        }
    }
    return GGML_BACKEND_KVARN_RECORD_LAYOUT_ROW_MAJOR;
}
...
llama_kv_cache_kvarn::llama_kv_cache_kvarn(
    ...
    for (uint32_t il = 0; il < hparams.n_layer_all; ++il) {
        ...
        const uint32_t layer_record_layout = llama_kvarn_backend_record_layout(dev);
        if (!record_layout_initialized) {
            record_layout = layer_record_layout;
            record_layout_initialized = true;
        } else if (record_layout != layer_record_layout) {
            throw std::runtime_error("KVarN cache layers use incompatible physical record layouts");
        }
...
void llama_kv_cache_kvarn::state_write(llama_io_write_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) const {
    ...
    io.write(&record_layout, sizeof(record_layout));
...
void llama_kv_cache_kvarn::state_read(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    uint32_t saved_record_layout = GGML_BACKEND_KVARN_RECORD_LAYOUT_ROW_MAJOR;
    if (version >= 17) {
        io.read(&saved_record_layout, sizeof(saved_record_layout));
    }
    if (saved_record_layout != record_layout) {
        throw std::runtime_error(format(
                "KVarN cache record layout mismatch: state has %u, cache has %u; "
                "re-save the prompt cache with this build",
                saved_record_layout, record_layout));
    }
